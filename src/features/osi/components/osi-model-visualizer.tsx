    ...
    <div className="grid gap-8">
      ...
      >
        <div className="grid gap-6 xl:grid-cols-[1fr_360px]">
          <section className="rounded-xl bg-background/30 p-5">
            <div className="mb-5 flex flex-wrap items-center justify-between gap-3">
              <h2 className="text-base font-semibold">OSI layers</h2>
                ...
                <button
                  ...
                  className={`relative overflow-hidden rounded-lg p-4 text-left transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring ${
                    selected.number === layer.number
                      ...
                      : "bg-secondary/18 hover:bg-secondary/32"
                  }`}
                >
                  {packetIndex === index ? (
                    <motion.div
                      layoutId="osi-packet"
                      className="absolute right-4 top-1/2 h-5 w-16 -translate-y-1/2 rounded-full bg-accent shadow-lg shadow-accent/15"
                    />
                  ) : null}
          ...
          <section className="rounded-xl bg-background/38 p-5">
            <h2 className="text-base font-semibold">Encapsulation view</h2>
            ...
            <div className="mt-6 grid gap-2">
              {["Data", "Segment", "Packet", "Frame", "Bits"].map((unit, index) => (
                <div
                  key={unit}
                  className="rounded-lg bg-secondary/22 p-3"
